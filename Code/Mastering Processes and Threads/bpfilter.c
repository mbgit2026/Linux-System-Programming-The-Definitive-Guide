...
static int install_filter(int syscall_nr, unsigned int t_arch, int f_errno)
{
    ...
    struct sock_filter filter[] = {
        ...
        /* [5] Matching architecture and system call: don't execute the system call, and return 'f_errno' in 'errno'.  */
...
{
    if (argc < 5) {
        fprintf(stderr, "Usage: ""%s <syscall_nr> <arch> <errno> <prog> [<args>]\n"
    ...
    if (install_filter(strtol(argv[1], NULL, 0), strtoul(argv[2], NULL, 0), strtol(argv[3], NULL, 0)))
        exit(EXIT_FAILURE);
